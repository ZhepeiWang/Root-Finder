// jt.cpp - Program for calculating the roots of a polynomial of real
// coefficients.
...
constexpr int kRpolyMaxDegree = 64;
...
void Fxshfr_ak1(int L2, int* NZ, double sr, double bnd, double K[MDP1], int N,
  ...
  double a, a1, a3, a7, b, betas, betav, c, d, e, f, g, h, oss, ots(0.0), otv(0.0), ovv,
...
void QuadIT_ak1(int N, int* NZ, double uu, double vv, double* szr, double* szi,
  ...
  double c, ee, mp, omp(0.0), relstp(0.0), t, u, ui, v, vi, zm;
...
void RealIT_ak1(int* iFlag, int* NZ, double* sss, int N, double p[MDP1], int NN,
  ...
  int i, j = 0, nm1 = N - 1;
  double ee, kv, mp, ms, omp(0.0), pv, s, t(0.0);
